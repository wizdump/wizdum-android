...
class RewardViewModel @Inject constructor(
    ...
    private val _rewardInfo = MutableStateFlow<RewardResponse>(RewardResponse())
    val rewardInfo: StateFlow<RewardResponse> = _rewardInfo

    fun getReward(lectureId: Int) {
        viewModelScope.launch {
            rewardRepository.getReward(lectureId).collect {
                _rewardInfo.value = it
            }
        }
    }
}